...
var Chart = function(context){
	...
	var Line = function(data,config,ctx){
		...
		if (!config.scaleOverride){
			...
			for (var i=0; i<calculatedScale.steps; i++){
				if(labelTemplateString){
				calculatedScale.labels.push(tmpl(labelTemplateString,{value:(config.scaleStartValue + (config.scaleStepWidth * (i+1))).toFixed(getDecimalPlaces (config.scaleStepWidth))}));
				}
			}
		}
